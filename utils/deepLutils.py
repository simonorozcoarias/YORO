...
dicc_size={0:1000,1:3000,2:2000,3:2000,4:1000,5:1000}
...
def YOLO_domain(optimizador=Adam,lr=0.001,momen=0,init_mode='glorot_normal',fun_act='linear',dp=0.2,regularizer=l2,w_reg=0,ventana=50000):
    ...
    layers = tf.keras.layers.Conv2D(22, (1, 10), strides=(1,1),padding='same',activation='sigmoid', use_bias=True, kernel_initializer=init_mode, bias_initializer='zeros', kernel_regularizer=regularizer(w_reg), bias_regularizer=None, activity_regularizer=None, kernel_constraint=None, bias_constraint=None)(L16)
    model = tf.keras.Model(inputs = inputs, outputs=layers)
    return model

def loadNNArchitecture(ventana,weigths_file):
    model = YOLO_domain(ventana=ventana)
  ...
  for index in range(Yhat.shape[0]):
    ...
    while cont>0:
      ...
      for i in ind_nonzero:
          ...
          size1=dicc_size[np.argmax(data_pred[0,ind_first,3:9])]
          size2=dicc_size[np.argmax(data_pred[0,i,3:9])]
...
def nt_TE(y,ventana,sample,threshold_presence,distancia=30):
  ...
  for i in range(y.shape[0]):
    ...
    for h in range(len(indices)):
      size = dicc_size[np.nonzero(y[i,0,indices[h],3:9]==np.amax(y[i,0,indices[h],3:9]))[0][0]]
